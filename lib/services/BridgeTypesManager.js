...
const { resolveRegistryAuth } = require('../utils/registryAuth');
    ...
    async attachNewBridgeTypesEntity(entity) {
        ...
        const pull = async () => {
            ...
            return this.doLockMutexAction(entity.id, async () => {
                ...
                try {
                    await docker.getImage(`${registry}:${newVersion}`).inspect();
                    imageExistsLocally = true;
                    this.debug.info('BridgeTypesManager.attachNewBridgeTypesEntity.pull', `Image ${registry}:${newVersion} found locally, skipping pull`);
                } catch (e) {
                    if (e.code !== 'ENOENT' && e.reason !== 'no such image' && e.statusCode !== 404) throw e;
                }

                if (!imageExistsLocally) {
                    const authconfig = resolveRegistryAuth(registry);

                    if (authconfig) {
                        this.debug.info('BridgeTypesManager.attachNewBridgeTypesEntity.pull', `Authenticating to ${authconfig.serveraddress} as ${authconfig.username}`);
                    }

                    const stream = await docker.pull(`${registry}:${newVersion}`, authconfig ? { authconfig } : {});
