        ...
        try {
            ...
            const reattachIfRestarting = async () => {
                try {
                    ...
                    const willRestart = policy && policy !== 'no' && (status === 'restarting' || status === 'running');

                    if (willRestart) {
                        this.debug.info('BridgesManager.attachContainer.reattach', `Container ${containerName} is ${status} with restart policy ${policy}; re-attaching.`);
                        bridge.containerInfo = info;
                        // attachContainer is a no-op while bridge.attached is still true,
                        // so detach() above must have already cleared the flag.
                        setImmediate(() => this.attachContainer(id).catch((err) => this.debug.warning('BridgesManager.attachContainer.reattach', err)));
                        return true;
                    }
                } catch (e) {
