...
class BridgesManager extends Base {
    ...
    async attachNewBridgeEntity(entity) {
        ...
        const start = async () => {
            ...
            // Pass the intended state explicitly: publishAttribute only writes to the
            // broker; the in-memory attribute read by getAttribute is refreshed
            // asynchronously when the echoed message is deserialized. Relying on
            // getAttribute here races and can make checkContainer read the stale
            // previous state (e.g. 'stopped' right after creating a bridge), which
            // sends it down the Stop branch and the container is never created.
            await this.checkContainer(entity.id, false, 'starting');
        };
        const stop = async () => {
            const state = entity.getAttribute('state');

            if (state !== 'stopping' && state !== 'stopped') await entity.publishAttribute('state', 'stopping', false);
            await this.checkContainer(entity.id, false, 'stopping');
        };
    ...
    async checkContainer(id, forceRecreation = false, desiredState = null) {
        ...
        try {
            ...
            const { entity } = bridge;
            // desiredState lets an explicit start/stop caller tell us the intended
            // lifecycle state without depending on the just-published attribute
            // having already round-tripped back into the in-memory entity.
            const state = desiredState || entity.getAttribute('state');
    ...
    async getRidOfTheContainer(id) {
        ...
        try {
            ...
            if (State.Status === 'running' || State.Status === 'restarting') {
                try {
                    // Disable restart policy first so Docker won't restart the container
                    // between stop() and remove(), which would cause a 409/timeout.
                    await docker.getContainer(containerName).update({ RestartPolicy: { Name: 'no' } });
                } catch (e) { /* ignore if update not supported */ }
